...
**To make use of this code, add** `#require "mLab.class.nut:1.0.0"` **to the top of your agent code.**

## Class Usage ##

### Constructor: mLab(*apiKey[, db]*) ###
...
- The *response* parameter is the [response table](https://developer.electricimp.com/api/httprequest/sendasync) returned from the request.
...
This method can be used to change the active database (ie. what database we’re running our queries against). This method should not be required in the majority of applications as the database should be set in the constructor.
...
This method lists all of the databases attached to the account associated with the API key.
...
This method lists all collections (tables) in the active database.
...
This method invokes a query against the specified collection.
...
This method inserts a new record into the specified collection.
...
This method updates a collection’s records that match the specified query.
...
local query = { "_id": { "$oid": "54d505afe4b05f6282260adf" } };

local newRecord = { "username": "newUserName",
                    "verified": false };
...
local query = { "verified": false };

// Add the verified flag, and set it to false
local updateAction = { "$set": { "verified": true } };
...
This method removes the specified record from the specified collection.
